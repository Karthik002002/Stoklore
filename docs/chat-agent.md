...
`POST /api/chat` (`app/routers/`) resolves the active model
...
No LangChain — `_OllamaDriver`/`_OpenAICompatDriver` (`app/core/llm.py:113`) each
normalize their backend's native tool-call response shape into one
`(assistant_msg, calls)` tuple. `run_agent_stream()` (`app/core/llm.py:195`) is the
...
"UI Message Stream" protocol (`_sse`, `app/routers/`): `start` →
...
`_guarded()` (`app/routers/`) so calling it returns
...
(`_wrap_tool_result`, `app/core/llm.py:176`). A regex (`_INJECTION_MARKERS`) flags
...
regexes in `app/main.py` + `app/routers/`, each resolved by its own reply helper before the

...
import type { ScriptResult } from '@/lib/runPineScript'
...
export default function AutoBacktestDetail() {
  ...
  const { data: saved } = useQuery({
    ...
    queryFn: () => getAutoBacktestScript(Number(scriptId)),
  })
  ...
  const save = useMutation({
    mutationFn: () => updateAutoBacktestScript(Number(scriptId), { name: name.trim(), script }),
    onSuccess: () => {
      ...
      <div className="space-y-3">
        ...
        {result &&
          ('trades' in result ? (
            ...
            <PlotsResultView plots={result.plots} />
          ))}
      </div>
    </div>
  )

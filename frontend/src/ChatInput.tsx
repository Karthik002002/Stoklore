...
import type { SymbolMatch } from '@/services/api'
import { PromptInputSubmit } from '@/components/ai-elements/prompt-input'

/** A slash command the agent exposes. */
type ChatCommand = { name: string; description: string; template: string }
/** A watch rule, taggable by name. */
type WatchRule = { id: number; name: string }
/** An event, taggable by its URL - a headline alone gives scrape_url nothing to fetch. */
type TaggableEvent = { headline: string; url: string | null }
/** What an @ menu entry inserts, and which icon labels it. */
type Tag = { type: 'stock' | 'rule' | 'event'; label: string; insertText: string }

const editorConfig = {
  namespace: 'ChatComposer',
  theme: {},
  onError: (error: Error) => console.error('Chat composer error:', error),
}

const TAG_TYPE_META: Record<Tag['type'], { icon: typeof TrendingUpIcon; label: string }> = {
...
class TextOption<T> extends MenuOption {
  data: T
...
function SlashMenuPlugin({
  ...
  const onSelectOption = useCallback(
    (option: TextOption<ChatCommand>, nodeToReplace: LexicalNode | null, closeMenu: () => void) => {
      editor.update(() => {
...
function TagMenuPlugin({
  ...
  const { data: stockMatches = [] } = useQuery({
    ...
    queryFn: (): Promise<SymbolMatch[]> =>
      fetch(`/api/stocks/search?q=${encodeURIComponent(queryString ?? '')}`).then((r) => r.json()),
  ...
  const { data: events = [] } = useQuery({
    ...
    queryFn: (): Promise<TaggableEvent[]> => fetch('/api/events?limit=1000').then((r) => r.json()),
  })
  const checkForTriggerMatch = useBasicTypeaheadTriggerMatch('@', { minLength: 0 })

  const options = useMemo(() => {
    const q = (queryString ?? '').toLowerCase()
    const stockTags: Tag[] = stockMatches.map((s) => ({
      type: 'stock',
      label: s.symbol,
      insertText: s.symbol,
    }))
...
type ChatInputProps = {
  ...
  open?: boolean
  /** Set by "Tag in chat" on an event card - replaces the input with that text. */
...
function ChatInputInner({
  ...
  const { data: rules = [] } = useQuery({
    queryKey: ['watchRules'],
    queryFn: (): Promise<WatchRule[]> => fetch('/api/watch-rules').then((r) => r.json()),
  })

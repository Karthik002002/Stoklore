...
import type { Goal, GoalPeriod } from '@/lib/tradeGoals'
import type { Trade } from '@/lib/types'
import { getTradingGoals, setTradingGoals } from '@/services/api'

const formatActual = (v: number | null | undefined, format?: string) => {
  if (v == null) return '—'
...
const heatFor = (pct: number | null | undefined) => {
...
const strokeFor = (pct: number | null | undefined) => {
...
function Gauge({ pct, size = 52 }: { pct: number | null | undefined; size?: number }) {
  const radius = 18
...
const asOptions = (record: Record<string, { label: string }>) =>
  Object.entries(record).map(([value, v]) => ({ value, label: v.label }))
...
function AddGoalForm({
  ...
  // The schema validates the shape but keeps `operator`/`mode` as plain strings; the select
  // options they come from are exactly the Goal unions, so this is where the two meet.
  const submit = (values: z.output<typeof goalSchema>) => {
    onAdd({ ...values, ...(values as Pick<Goal, 'operator' | 'mode'>), id: crypto.randomUUID(), period })
    form.reset(BLANK)
  }
...
function CurrentPeriod({
  ...
  progress: ReturnType<typeof currentPeriodProgress>
  period: GoalPeriod
}) {
  return (
...
function HistoryTable({
  ...
  evaluated: ReturnType<typeof evaluateGoals>
          ...
          {evaluated.rows.map((row) => (
            <tr key={row.key} className="border-b last:border-0">
                  ...
                  <td key={cell.goalId} className="px-1 py-1">
                    <div className={`rounded-md px-2 py-1.5 ${heatFor(cell.pct)}`}>
                      ...
                      <span className="ml-2 text-xs text-muted-foreground tabular-nums">
                        {formatActual(cell.actual, goal && GOAL_METRICS[goal.metric]?.format)}
                      </span>
                    </div>
                  </td>
                )
              ...
              <td className="sticky right-0 z-10 bg-card px-3 py-2 text-right">
                <span
                  className={`font-semibold tabular-nums ${(row.total ?? 0) >= 60 ? 'text-up' : 'text-down'}`}
                >
...
export default function ManualGoals({ trades }: { trades: Trade[] }) {
...
export function GoalsSummary({ trades }: { trades: Trade[] }) {
  const { goals } = useGoals()
  if (goals.length === 0) return null

  const periods = (Object.keys(PERIODS) as GoalPeriod[]).filter((p) => goals.some((g) => g.period === p))

...
/** One day's trading, as the calendar shades it. */
type DayTally = { pnl: number; wins: number; losses: number }
/** One row of a "P&L by X" breakdown. */
type BreakdownRow = { label: string; metricValue: number; count: number; winRate: number }
/** One row of the concentration lists: how much of the total sits on one name. */
type ConcentrationRow = { label: string; amount: number; count: number; pctOfTotal: number }
/** One setup's R spread. */
type RSpreadRow = { label: string; count: number; min: number; median: number; max: number }
...
function MiniChart({ data, kind }: { data: { time: string; value: number }[]; kind?: string }) {
...
function CalendarHeatmap({
  ...
  dailyByDay: Map<string, DayTally>
...
function computeBreakdownRows(trades: Trade[], keyFn: (t: Trade) => string | null | undefined) {
  ...
  return [...groups.entries()].map(([label, group]) => {
    const groupPnls = group.map((t) => tradePnl(t) ?? 0)
...
function sortByOrder<T extends { label: string }>(rows: T[], order: string[]) {
...
function RDistributionCard({ rows }: { rows: { bucket: string; count: number }[] }) {
...
function computeConcentration(trades: Trade[], keyFn: (t: Trade) => string | null | undefined) {
  const total = trades.reduce((s, t) => s + Math.abs(tradePnl(t) ?? 0), 0)
  const groups = new Map<string, Trade[]>()
  trades.forEach((t) => {
    const key = keyFn(t) || 'Untagged'
    if (!groups.has(key)) groups.set(key, [])
  ...
  return [...groups.entries()]
    .map(([label, group]) => {
      const amount = Math.round(group.reduce((s, t) => s + Math.abs(tradePnl(t) ?? 0), 0) * 100) / 100
      return {
        label,
        count: group.length,
        amount,
...
function computeTagLossFrequency(losingTrades: Trade[]) {
  const totalLoss = losingTrades.reduce((s, t) => s + Math.abs(tradePnl(t) ?? 0), 0)
  const byTag = new Map<string, { count: number; amount: number }>()
  losingTrades.forEach((t) => {
    const lossAbs = Math.abs(tradePnl(t) ?? 0)
    ;(t.tags ?? []).forEach((tag) => {
...
function ConcentrationList({ rows, tone }: { rows: ConcentrationRow[]; tone?: 'up' | 'down' }) {
...
const CONCENTRATION_DIMENSIONS: Record<
  string,
  { label: string; keyFn: (t: Trade) => string | null | undefined }
> = {
...
function ConcentrationCard({
  ...
  const [dimension, setDimension] = useState<keyof typeof CONCENTRATION_DIMENSIONS>('setup')
  const rows = useMemo(
  ...
  return (
    <div className="rounded-xl border bg-card p-4">
      <div className="mb-3 flex items-center justify-between">
        ...
        <Select value={dimension} onValueChange={(v) => setDimension(String(v))}>
...
function TagLossCard({ losingTrades }: { losingTrades: Trade[] }) {
...
function computeRSpreadBySetup(riskTrades: Trade[]) {
  ...
  return [...groups.entries()]
    .map(([label, rs]) => {
      ...
      return { label, count: rs.length, min: sorted[0], median, max: sorted.at(-1) ?? sorted[0] }
    })
    .sort((a, b) => b.median - a.median)
}

function RSpreadCard({ rows }: { rows: RSpreadRow[] }) {
  const clamp = (r: number) => Math.max(-3, Math.min(3, r))
  const pos = (r: number) => ((clamp(r) + 3) / 6) * 100
  return (
    <div className="rounded-xl border bg-card p-4">
...
function MonthlyReturnsGrid({ monthlyByMonth }: { monthlyByMonth: Map<string, number> }) {
...
/** One line on a Plot. */
type PlotSeries = { label?: string; color?: string; dashed?: boolean; points: { x: number; value: number }[] }
...
function Plot({
  ...
  series: PlotSeries[]
  /** Shaded x-ranges: the losing streak, the sweet spot. */
  bands?: { from: number; to: number; color: string; label?: string }[]
  /** A labelled dot - where the user's own numbers actually sit. */
  markers?: { x: number; value: number; label: string; color?: string }[]
  xTicks?: number[]
  yTicks?: number[]
  ...
  const sx = (x: number) => PLOT.padL + ((x - x0) / (x1 - x0 || 1)) * (PLOT.w - PLOT.padL - PLOT.padR)
  const sy = (y: number) =>
    PLOT.h - PLOT.padB - ((y - y0) / (y1 - y0 || 1)) * (PLOT.h - PLOT.padT - PLOT.padB)
...
function ExpectancyGridCard({ winRate, payoff }: { winRate: number; payoff: number }) {
  ...
  return (
    ...
    >
      <Plot
        ...
        yTicks={[Math.floor(sim.trough / 10) * 10, 100, Math.ceil((sim.points.at(-1)?.value ?? 0) / 10) * 10]}
        formatY={(v) => `${v}%`}
      />
...
function RiskPathsCard({ winRate, payoff }: { winRate: number; payoff: number }) {
  const paths = useMemo(
...
function AchievableWinRateCard({ winRate, payoff }: { winRate: number; payoff: number }) {
...
function CompoundingCard({ gainPct, lossPct }: { gainPct: number; lossPct: number }) {
...
function DrawdownRiskCard({ winRate, payoff }: { winRate: number; payoff: number }) {
  const rows = useMemo(
...
function BalanceAdjusterDialog({
  open,
  onOpenChange,
  adjustments,
  accountId,
  ...
  const create = useMutation({
    mutationFn: (values: z.output<typeof balanceAdjustmentSchema>) =>
      createBalanceAdjustment({
...
export default function ManualOverview({ trades, accountId }: { trades: Trade[]; accountId: number | null }) {
  ...
  // Closed trades only, so every P&L is a real number - the null case is an open position.
  const pnls = useMemo(() => closed.map((t) => tradePnl(t) ?? 0), [closed])
  ...
  const losingTrades = useMemo(() => closed.filter((t) => (tradePnl(t) ?? 0) < 0), [closed])
  ...
  const revengeLoss = Math.abs(revengeTrades.reduce((s, t) => s + Math.min(tradePnl(t) ?? 0, 0), 0))
  ...
  const winningTrades = useMemo(() => closed.filter((t) => (tradePnl(t) ?? 0) > 0), [closed])
  ...
  const top3Profit = [...winningTrades]
    .sort((a, b) => (tradePnl(b) ?? 0) - (tradePnl(a) ?? 0))
    .slice(0, 3)
  ...
  const bestTrade = closed.length
    ? closed.reduce((best, t) => ((tradePnl(t) ?? 0) > (tradePnl(best) ?? 0) ? t : best))
    : null
  const worstTrade = closed.length
    ? closed.reduce((worst, t) => ((tradePnl(t) ?? 0) < (tradePnl(worst) ?? 0) ? t : worst))
    : null
  ...
  const model = useMemo(() => {
    ...
    const plannedRisk = riskTrades.length
      ? riskTrades.reduce((s, t) => s + (t.ideal_risk_amount ?? 0), 0) / riskTrades.length
      : null
      ...
      {riskTrades.length > 0 ? (
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          ...
          <StatCard
            ...
            valueClassName={(avgExpectedR ?? 0) >= 0 ? 'text-up' : 'text-down'}
          />
          <StatCard

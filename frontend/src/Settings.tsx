...
import type { RuleCheck, WatchRule } from '@/services/api'
import {
  addStock,
  checkWatchRule,
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-2">
        ...
        <Select value={active?.model ?? ''} onValueChange={(model) => model && save.mutate(String(model))}>
...
function BrokerTab() {
  ...
  const setBroker = (next: string) =>
    navigate({
      search: ((prev: Record<string, unknown>) => ({ ...prev, broker: next })) as never,
      replace: true,
    })
...
function WatchRulesTab() {
  ...
  const [results, setResults] = useState<Record<number, RuleCheck | RuleCheck[]>>({})
  ...
  const create = useMutation({
    mutationFn: ({ name, criteria }: { name: string; criteria: string }) =>
      createWatchRule({ name, text: criteria }),
    onSuccess: ({ criteria }) => {
  ...
  const check = async (rule: WatchRule) => {
    try {
...
function DataCollectionTab() {
  ...
  const removeSymbol = (symbol: string) => setSymbols((s) => s.filter((x) => x !== symbol))
...
function ActivityTab() {
  ...
  const [qualifiers, setQualifiers] = useState<Record<string, boolean> | null>(null)
  ...
  const save = useMutation({
    mutationFn: () =>
      setActivitySettings({ qualifiers: qualifiers ?? {}, daily_goal_minutes: Number(goalMinutes) }),
    onSuccess: () => {
  ...
  const toggle = (key: string) => setQualifiers((q) => ({ ...q, [key]: !q?.[key] }))
...
function WatchlistCell({
  ...
  memberOf: Set<string>
  tracked: boolean
  onAdd: (symbol: string, listName: string, tracked: boolean) => void
  onRemove: (symbol: string, listName: string) => void
  onCreateList: (symbol: string, tracked: boolean) => void
}) {
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
...
function StocksMasterTab() {
  ...
  // undefined = both boards; the buttons set 'MAIN' or 'SME'.
  ...
  const addToWatchlist = async (symbol: string, listName: string, tracked: boolean) => {
    if (!tracked) {
      try {
  ...
  const removeFromWatchlist = async (symbol: string, listName: string) => {
  ...
  const createListAndAdd = (symbol: string, tracked: boolean) => {
  ...
  const importCsv = useMutation({
    mutationFn: (file: File) => importStocksMaster(file),

...
const colorFor = (i: number) => ACCOUNT_COLORS[i % ACCOUNT_COLORS.length]

const PRESET_KEY = 'tradeSimulation.preset'

/** What both tabs are handed: the shared config, and the journal to run it against. */
/** Merges a patch into the config - the page owns the whole object. */
type SetConfig = (patch: Partial<PresetConfig>) => void
/** An account plus the label the picker shows for which book it is (see the page's `accounts`). */
type LabelledAccount = TradeAccount & { kindLabel: string }
/** One account's finished run: what the comparison charts need, plus the daily series the
 *  correlation matrix is built from. */
type AccountRun = ComparisonEntry & {
  id: number
  total: number
  pnls: number[]
...
/** The trade pool the config is being applied to (poolStats). */
type Pool = ReturnType<typeof poolStats>
...
const TABLE_ROWS: { key: string; label: string; format: (v: number | null) => string }[] = [
  { key: 'endBalance', label: 'Ending account balance', format: inr },
  { key: 'maxDD', label: 'Max absolute drawdown', format: (v: number | null) => `${fmt(v, 1)}%` },
  { key: 'maxLossStreak', label: 'Max consecutive losses', format: (v: number | null) => fmt(v, 0) },
  { key: 'roi', label: 'Return on investment', format: (v: number | null) => `${fmt(v, 1)}%` },
  {
    key: 'profitFactor',
    label: 'Profit factor',
    format: (v: number | null) => ((v ?? 0) >= 1e8 ? '∞' : fmt(v, 2)),
  },
  { key: 'sharpe', label: 'Sharpe (per trade)', format: (v: number | null) => fmt(v, 2) },
]
...
/** One line: its points, its colour, and whatever the caller wants back in the tooltip. */
// ArrayLike, not number[]: the engine works in Float64Array and the canvas only indexes.
type Path = { data: ArrayLike<number>; color?: string; label?: string; [key: string]: unknown }
/** A path once the canvas has decided how to draw it. */
type DrawnPath = Path & { color: string; lineWidth: number }
/** The scales from the last paint, kept so hit-testing doesn't redo the layout. */
type Geometry = {
  sx: (i: number) => number
  sy: (v: number) => number
  steps: number
...
function PathsCanvas({
  ...
  /** The bundle of individual runs, drawn thin. */
  faint?: Path[]
  /** The percentile lines drawn over them, each with its own colour. */
  bold?: Path[]
  log?: boolean
  /** Draws downward from the top - used for the drawdown chart. */
  invert?: boolean
  height?: number
  formatY: (v: number) => string
  tooltip?: (path: DrawnPath) => React.ReactNode
}) {
  const wrapRef = useRef<HTMLDivElement>(null)
  const canvasRef = useRef<HTMLCanvasElement>(null)
  const [width, setWidth] = useState(0)
  // The line under the cursor, and where its point sits, or null.
  const [hover, setHover] = useState<{ i: number; x: number; y: number } | null>(null)
  // Scales from the last paint, so hit-testing doesn't recompute the layout.
  const geom = useRef<Geometry | null>(null)

  const series = useMemo<DrawnPath[]>(
    () => [
      ...faint.map((s) => ({ ...s, color: FAINT, lineWidth: 1 })),
      ...bold.map((b) => ({ ...b, color: b.color ?? FAINT, lineWidth: 2 })),
    ],
    [faint, bold],
  )
  ...
  useEffect(() => {
    ...
    const t = log ? (v: number) => Math.log10(Math.max(v, 1)) : (v: number) => v
    const tLo = t(lo)
    const tHi = t(hi)
    const steps = Math.max(...bold.map((b) => b.data.length))
    const sx = (i: number) => padL + (i / (steps - 1 || 1)) * (width - padL - padR)
    const sy = (v: number) => {
    ...
    const stroke = (data: ArrayLike<number>, color: string, lineWidth: number) => {
  ...
  const onMove = (e: React.MouseEvent) => {
    if (!tooltip || !geom.current || !canvasRef.current) return
    ...
    const hit = best as { d: number; i: number; x: number; y: number } | null
    setHover((prev) => (hit ? { i: hit.i, x: hit.x, y: hit.y } : prev === null ? prev : null))
  }
    ...
    <div ref={wrapRef} className="relative w-full">
      ...
      {hovered && tooltip && (
        <div
          ...
          style={{
            left: Math.min((hover?.x ?? 0) + 12, Math.max(width - 220, 0)),
            top: Math.min((hover?.y ?? 0) + 12, height - 40),
          }}
        >
          {tooltip(hovered)}
        </div>
...
function SlippagePanel({ config, set, pool }: { config: PresetConfig; set: SetConfig; pool?: Pool | null }) {
...
function ConfigSidebar({
  ...
  set: SetConfig
  pool?: Pool | null
  /** The slice of the log the run covers, as tradeRange reports it. */
  range?: ReturnType<typeof tradeRange> | null
  rangeHint?: React.ReactNode
  onRun: () => void
  canRun: boolean
  runLabel: string
  onCsv: () => void
  csvLabel: string
  hasResult: boolean
  /** The markdown report, built only when there is a result to describe. */
  md?: () => string
  mdName?: string
}) {
  const savePreset = () => {
        ...
        <div className="space-y-3">
          ...
          <Field label="Length (trades to project)">
            <div className="flex gap-1.5">
              ...
              <Button
                ...
                variant="outline"
                disabled={!pool}
                onClick={() => pool && set({ length: pool.n })}
              >
                Match log
              </Button>
            </div>
          ...
          >
            <Choice
              options={MODELS}
              value={config.model}
              onChange={(v) => set({ model: v as PresetConfig['model'] })}
            />
          </Field>
        </div>
      </Panel>
      ...
      <Panel title="Position sizing">
        <div className="space-y-3">
          <Choice
            ...
            onChange={(v) => set({ sizingMode: v as PresetConfig['sizingMode'] })}
          />
...
function DataProfile({ pool, hint }: { pool?: Pool | null; hint?: React.ReactNode }) {
  return (
      ...
      {pool ? (
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6">
          ...
          <StatCard
            ...
            valueClassName={(pool.profitFactor ?? Infinity) >= 1 ? 'text-up' : 'text-down'}
          />
    ...
    <>
      ...
      {TABLE_ROWS.map((row) => (
        <div key={row.key} className="flex justify-between gap-2">
          ...
          <span className="tabular-nums">{row.format(stats[row.key as keyof typeof stats])}</span>
        </div>
      ))}
    </>
  )
...
function SimulationResults({ result, config }: { result: SimulationResult; config: PresetConfig }) {
    ...
    <>
      ...
      >
        <PathsCanvas
          ...
          log={logEquity}
          formatY={(v) => compact(v)}
          tooltip={(s) => <RunTooltip result={result} run={s.run as number} label={s.label} />}
        />
        <Legend
          items={[
          ...
          <TableBody>
            {TABLE_ROWS.map((row) => (
              <TableRow key={row.key}>
                ...
                {result.table[row.key as keyof typeof result.table].map((v, i) => (
...
function correlationTint(r: number | null) {
  if (r == null) return 'transparent'
  const a = Math.min(Math.abs(r), 1) * 0.55
  return r >= 0 ? `rgba(34, 197, 94, ${a})` : `rgba(239, 68, 68, ${a})`
}

function CorrelationMatrixPanel({ entries, matrix }: { entries: AccountRun[]; matrix: CorrelationCell[][] }) {
  return (
    <Panel
              ...
              <TableBody>
                {entries.map((e, i) => (
                  <TableRow key={e.id}>
                    ...
                    {e.result.table[row.key as keyof typeof e.result.table].map((v, k) => (
                      // biome-ignore lint/suspicious/noArrayIndexKey: fixed 5-percentile columns
...
const buildSizing = (config: PresetConfig): Sizing =>
  config.sizingMode === 'fixed-amount'
    ? { mode: 'fixed-amount', amount: config.riskAmount ?? 0 }
    : config.sizingMode === 'fixed-pct'
      ? { mode: 'fixed-pct', pct: config.riskPct ?? 0 }
      : { mode: 'as-logged' }

const runFor = (pnls: number[], config: PresetConfig) =>
  simulate({
    pnls,
...
function SingleMode({ config, set, accounts, allTrades }: ModeProps) {
  ...
  const pnls = useMemo(
    () => (range.error ? [] : closedTrades.slice(range.start, range.end).map((t) => tradePnl(t) ?? 0)),
    [closedTrades, range],
  )
      ...
      {account == null ? (
        ...
        <div className="flex flex-col gap-4 lg:flex-row">
          <ConfigSidebar
            ...
            csvLabel="CSV"
            hasResult={!!result}
            onCsv={() => result && downloadCsv(toCsv(result), `simulation-${selected?.name ?? 'log'}`)}
            md={() => (result ? toMd(result, { title: selected?.name ?? 'log', config }) : '')}
...
function MultiMode({ config, set, accounts, allTrades }: ModeProps) {
  const { accounts: idsParam, view } = useSearch({ from: '/simulation' })
  const navigate = useNavigate({ from: '/simulation' })
  // Keyed by account id: the run for each account on the board, or null before the first run.
  const [results, setResults] = useState<Record<number, AccountRun> | null>(null)
  ...
  const chosen = useMemo(
    () => ids.map((id) => accounts.find((a) => a.id === id)).filter((a): a is LabelledAccount => a != null),
    [ids, accounts],
  )
  ...
  const run = () => {
    setResults(
      Object.fromEntries(runnable.map((a) => [a.id, { ...a, result: runFor(a.pnls, config) }])) as Record<
        number,
        AccountRun
      >,
    )
...
export default function TradeSimulation() {
  ...
  const set: SetConfig = (patch) => setConfig((c) => ({ ...c, ...patch }))

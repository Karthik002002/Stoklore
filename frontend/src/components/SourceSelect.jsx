...
const SOURCE_LABELS = { yfinance: 'Yahoo Finance', moneycontrol: 'Moneycontrol' }
...
export default function SourceSelect({ sources, value, onChange, className }) {
  if (sources.length <= 1) return null
  return (
    <Select value={value} onValueChange={onChange}>
      <SelectTrigger size="sm" className={className}>
        <SelectValue>{(v) => SOURCE_LABELS[v] ?? v}</SelectValue>
      </SelectTrigger>
      <SelectContent>
        {sources.map((s) => (

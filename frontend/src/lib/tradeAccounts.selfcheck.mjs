...
import { accountBalance, accountReturnPct, capWarnings, positionSizeCap, tradesForAccount } from './tradeAccounts.js'
...
assert.deepEqual(tradesForAccount(trades, 1).map((t) => t.id), [1])
...
assert.equal(positionSizeCap({ ...account, max_position_size_type: 'percentage', max_position_size: 5 }, 100000), 5000)
assert.equal(
...
// --- warnings -----------------------------------------------------------------------------------
assert.deepEqual(capWarnings(null, { positionValue: 999999, openCount: 99, balance: 1 }), [], 'no account, no rules')
assert.equal(capWarnings(account, { positionValue: 15000, openCount: 1, balance: 100000 }).length, 0, 'inside both caps')
assert.equal(capWarnings(account, { positionValue: 25000, openCount: 0, balance: 100000 }).length, 1, 'over size')

...
// under plain `node`.
import { actualRiskAmount, lossExceededStop, tradePnl, underwaterSeries } from './manualTrades.js'
import { METRICS } from './tradeStats.js'
...
export const GOAL_METRICS = {
  ...METRICS,
  wins: { label: 'Winning trades', format: 'num', of: (g) => g.filter((t) => tradePnl(t) > 0).length },
  losses: { label: 'Losing trades', format: 'num', of: (g) => g.filter((t) => tradePnl(t) < 0).length },
  grossLoss: {
    label: 'Gross loss',
    format: 'inr',
    of: (g) => round(Math.abs(sum(g.map(tradePnl).filter((p) => p < 0)))),
  },
  maxDrawdown: {
    label: 'Max drawdown',
    format: 'inr',
    of: (g) => {
      let running = 0
      return underwaterSeries(g.map((t) => (running += tradePnl(t)))).maxDrawdown
    },
  },
...
export function periodKey(tradedAt, period) {
...
export function achievement(goal, actual) {
...
function scoreCell(goal, trades) {
  const metric = GOAL_METRICS[goal.metric]
...
export function evaluateGoals(trades, goals, period) {

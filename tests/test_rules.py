...
def test_no_negative_events_check():
    ...
    with patch("app.core.db.list_events", return_value=[{"sentiment_label": "negative"}, {"sentiment_label": "positive"}]):
        result = rules.evaluate(rule, "TEST")
    assert result["passed"] is False
...
def test_parse_watch_rule_extracts_only_mentioned_fields():
    ...
    with patch("app.core.llm._generate", return_value='Sure, here it is: {"max_pe": 30, "ema_short": null}\nhope that helps'):
